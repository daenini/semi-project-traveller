...
int count = 0;
count = productService.getCount(); // 데이터베이스에 저장된 총 갯수

int pageSize = 12; // 한 페이지에 출력 할 레코드 수
...
<html lang="kr">
    ...
    <jsp:include page="/include/top_header_include.jsp"></jsp:include>
                        ...
                        <div class="card h-100">
                            ...
                            <div class="card-body p-4">
                                <div class="text-center">
                                    ...
                                    <h6 class="fw-bolder"><%=product.getP_name()%></h6>
                                    <!-- Product price-->
                                    <h7><%=new DecimalFormat("#,##0").format(product.getP_price())%>원</h7>
                                </div>
                            </div>
                            <!-- Product actions-->
                            <form id = "cart_form_<%= product.getP_no() %>">
                            <input type='hidden' name='c_qty' value=1 size=4 class=TXTFLD>
		...
		<div class="text-center">
				...
				%>
				<button id="subBtn" type="button" class="btn btn-primary" onclick="location.href='p_list.jsp'" />전체보기</button>
				<button id="subBtn" type="button" class="btn btn-primary" onclick="location.href='p_insert.jsp'" />상품등록</button>
				<%
				}
				%>
			</div>	 
